...
Column todo  = new Column("To Do", "#FFAAAA", 1);
Column doing = new Column("Doing", "#FFD580", 2);
Column done  = new Column("Done", "#AAFFAA", 3);
...
{
    KanbanTest.getTaskColumn().put(todo,  new ArrayList<>());
    KanbanTest.getTaskColumn().put(doing, new ArrayList<>());
    KanbanTest.getTaskColumn().put(done,  new ArrayList<>());

    KanbanTest.getTaskColumn().get(todo).add(t1);
    KanbanTest.getTaskColumn().get(todo).add(t2);
    ...
    try {
        ...
        */
        ...
        // ✅ Test direct du nouvel écran kanban :
        client.ihmKanban.kanbanCorps coreKanban = new client.ihmKanban.kanbanCorps();
        ManageDisplay display = new ManageDisplay(coreKanban);
        display.openKanbanScreen(KanbanTest);

        System.out.println("[MainCore] Launched test Kanban window");

    } catch (Exception e) {
        System.err.println("Error while launching main window: " + e.getMessage());
        throw new RuntimeException("Impossible d’ouvrir la fenêtre de test Kanban", e);
    }
}
